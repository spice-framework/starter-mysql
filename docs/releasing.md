# Releasing starter-mysql

This library owns its release construction. It does not use GoReleaser or a
second dependency/build graph: `cmd/starter-mysql-release` reads the
...
- `starter-mysql_0.1.0_source.tar.gz`, containing every file in the exact
  committed `HEAD` tree under the single `starter-mysql-0.1.0/` prefix;
- `starter-mysql_0.1.0_sbom.spdx.json`, an SPDX 2.3 document for the root
...
```text
go run ./cmd/starter-mysql-release -rehearsal -version v0.1.0-rc.1 -output dist-rehearsal
```
...
`starter-mysql-VERSION/` and `starter-mysql_VERSION/`, respectively. Parity
...
- document name (`Spice MySQL Starter VERSION` retained and
  `starter-mysql VERSION` centrally);
...
deliberately continues to invoke `cmd/starter-mysql-release`, validate its
...
```text
openssl genpkey -algorithm ED25519 -out starter-mysql-release-key.pem
```

The command also accepts a base64-encoded 32-byte Ed25519 seed or 64-byte
private key. GitHub reads the protected `SPICE_MYSQL_RELEASE_SIGNING_KEY` secret. The
...
could replace both. Until a reviewed starter-mysql public-key fingerprint is
