...
func TestValidateReleaseArchiveParity(t *testing.T) {
	...
	writeParityTestArchive(t, central, "starter-mysql_0.0.0-rehearsal/", "same", "")
	writeParityTestArchive(t, retained, "starter-mysql-0.0.0-rehearsal/", "same", "")
	if err := validateReleaseArchiveParity(
		central,
		"starter-mysql_0.0.0-rehearsal/",
		retained,
		"starter-mysql-0.0.0-rehearsal/",
	); err != nil {
	...
	t.Run("rejects entry drift", func(t *testing.T) {
		...
		writeParityTestArchive(t, drifted, "starter-mysql-0.0.0-rehearsal/", "different", "")
	...
	t.Run("rejects unexpected retained root", func(t *testing.T) {
		...
		writeParityTestArchive(t, wrong, "starter-mysql_0.0.0-rehearsal/", "same", "")
		err := validateReleaseArchiveParity(
			central,
			"starter-mysql_0.0.0-rehearsal/",
			wrong,
	...
	t.Run("rejects hidden decompressed tail", func(t *testing.T) {
		...
		prefix := "starter-mysql_0.0.0-rehearsal/"
...
func TestValidateReleaseSBOMParity(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "documented R1 differences"},
		{
			name: "package drift",
			mutate: func(_ *releaseSBOM, retained *releaseSBOM) {
				retained.Packages[0].VersionInfo = "v9.9.9"
			},
			wantErr: "outside documented R1",
		},
		{
			name: "wrong MySQL provenance",
			mutate: func(_ *releaseSBOM, retained *releaseSBOM) {
				retained.CreationInfo.Creators[0] = "Organization: Spice Authors"
			},
			wantErr: "MySQL builder",
		},
		{
			name: "retained describes",
			mutate: func(_ *releaseSBOM, retained *releaseSBOM) {
			...
			wantErr: "must not contain DESCRIBES",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func paritySBOMFixtures() (releaseSBOM, releaseSBOM) {
	...
	central := common
	central.Name = "starter-mysql " + rehearsalVersion
	...
	retained := common
	retained.Name = "Spice MySQL Starter " + rehearsalVersion
	...
	retained.Packages = []sbomPackage{dependency, root}
	retained.Relationships = []sbomRelationship{dependsOn}
...
func writeParityTestRelease(
	...
	t.Helper()
	root := t.TempDir()
	prefix := "starter-mysql-0.0.0-rehearsal/"
...
func releaseArchiveName() string {
	return "starter-mysql_" + strings.TrimPrefix(rehearsalVersion, "v") + "_source.tar.gz"
}

func releaseSBOMName() string {
	return "starter-mysql_" + strings.TrimPrefix(rehearsalVersion, "v") + "_sbom.spdx.json"
}

func parityNamespace(extra string, digit rune) string {
	return "https://github.com/spice-framework/starter-mysql/releases/" + rehearsalVersion +
