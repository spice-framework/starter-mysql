...
func releaseRehearsal(ctx context.Context, root string) error {
	parent, err := os.MkdirTemp("", "starter-mysql-release-rehearsal-*")
	if err != nil {
	...
	plan, err := capture(ctx, root, offlineVendor, "go", "tool", developmentTool,
		"library-release", "plan", "--root="+root, "--repo=starter-mysql",
...
func validateReleaseSBOM(content []byte) error {
	...
	prefix := "https://github.com/spice-framework/starter-mysql/releases/" + rehearsalVersion + "/spdx/v1/"
	if sbom.SPDXVersion != "SPDX-2.3" || sbom.DataLicense != "CC0-1.0" || sbom.SPDXID != "SPDXRef-DOCUMENT" ||
		sbom.Name != "starter-mysql "+rehearsalVersion || !validSBOMNamespace(sbom.DocumentNamespace, prefix) ||
