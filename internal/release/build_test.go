...
func TestSourceArchivePreservesCommittedExecutableAndSymlink(t *testing.T) {
	...
	executableHeader, executableFound := headers["starter-mysql-0.1.0/run.sh"]
	linkHeader, linkFound := headers["starter-mysql-0.1.0/run-link"]
...
func verifySourceArchive(t *testing.T, directory, root string, epoch time.Time) {
	t.Helper()
	archivePath := filepath.Join(directory, "starter-mysql_0.1.0-rc.1_source.tar.gz")
	...
	for {
		...
		if !strings.HasPrefix(header.Name, "starter-mysql-0.1.0-rc.1/") || filepath.IsAbs(header.Name) {
			t.Fatalf("unsafe archive path %q", header.Name)
		}
		found[strings.TrimPrefix(header.Name, "starter-mysql-0.1.0-rc.1/")] = true
	}
...
func verifySBOM(t *testing.T, directory, root string, epoch time.Time) {
	t.Helper()
	data := readTestFile(t, filepath.Join(directory, "starter-mysql_0.1.0-rc.1_sbom.spdx.json"))
